...
def init_halfmoons(n_nodes, noise):
    nodes, labels_true = noisy_moons = datasets.make_moons(n_samples=n_nodes, noise=noise)
    return nodes, labels_true
...
def create_moons(n_noes, noise, k):
    nodes, labels = init_halfmoons(n_nodes, noise)
    G = cluster(nodes, k)
    coordinates = dict(list(enumerate(map(tuple, nodes)))) # a dict that maps to the cooordinates of each node, for plotting
    return G, coordinates, labels
